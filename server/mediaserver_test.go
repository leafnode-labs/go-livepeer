...
func TestRegisterConnection(t *testing.T) {
	...
	drivers.NodeStorage = nil
	_, err := s.registerConnection(context.TODO(), strm, nil, nil)
	assert.Equal(err, errStorage)
	drivers.NodeStorage = drivers.NewMemoryDriver(nil)

	// normal success case
	rand.Seed(123)
	cxn, err := s.registerConnection(context.TODO(), strm, nil, nil)
	assert.NotNil(cxn)
	assert.Nil(err)
	...
	inCodec := ffmpeg.H264
	strm = stream.NewBasicRTMPVideoStream(&core.StreamParameters{ManifestID: core.RandomManifestID(), Profiles: profiles})
	cxn, err = s.registerConnection(context.TODO(), strm, &inCodec, nil)
	assert.Nil(err)
